...
const data = {
    ...
    items: [
        "3D Visualization", "Visual Effects", "Post Production",
        "Retouching", "Pitch Deck Design", "Web Development"
    ]
}
...
const BlockServices = ({innerRef}) => (
    <div ref={innerRef} className={s.services}>
        ...
        <div className={s.listL}>
            <ul>{data.items.slice(0, 3).map((item, i) => (
                <li key={i}>{item}</li>))}
            </ul>
        </div>
        <div className={s.listR}>
            <ul>{data.items.slice(3).map((item, i) => (
                <li key={i}>{item}</li>))}
            </ul>
        </div>
